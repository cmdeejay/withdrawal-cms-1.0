...
def main():
    ...
    with sqlite3.connect("../trading_account/trading_account.sqlite") as conn:
        exisiting_client_ids = pd.read_sql_query(
            """
                                SELECT client_id FROM client_balance""",
            conn,
        )
    exisiting_client_ids = exisiting_client_ids["client_id"].tolist()
    client_ids = list(set(client_ids) - set(exisiting_client_ids))
